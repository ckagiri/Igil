    ...
    function($, _, ko, datacontext, router, eventDelegates, utils, messenger, config, store, SeasonsViewModel) {
        var isBusy = false,
            ...
            resultTemplate = function(item) {
                return selectedFixtureItem() === item ? 'result.edit' : 'result.view';
            },
            ...
            gotoDetails = function (selectedFixture) {
                if (selectedFixture && selectedFixture.id()) {
                    router.navigateTo(config.hashes.results + '/' + selectedFixture.id());
                }
            },
            init = function () {
                eventDelegates.resultsEdit(gotoDetails);
            },
