...
function () {
    var Init = function(form) {
        var selection = form.selection,
            ...
            diff,
            output = "",
            pendingOutput = "Bet " + selection + " " + Math.abs(handicap) + " goal",
            ending = s_handicap.substr((s_handicap.length - 2), 2); // ?? math refactor
        if (Math.abs(handicap) != 1) {
        ...
        if (allScenarios) {
            ...
            for (var i = 0; i < diffScenarios.length; i++) {
                if (diffScenarios[i] == 0) {
                    pendingOutput = pendingOutput + " If the match ends goal-less, ";
                } else if (diffScenarios[i] < 0) {
                    pendingOutput = pendingOutput + " If the match ends with " + Math.abs(diffScenarios[i]);
                    if (i == 0) {
                        pendingOutput = pendingOutput + " or more goals";
                    }
                    if (i == (diffScenarios.length - 1)) {
                        if (Math.abs(diffScenarios[i]) != 1) {
                            pendingOutput = pendingOutput + " goals or less, ";
                        } else {
                            pendingOutput = pendingOutput + " goal or ends goal-less, ";
                        }
                    } else {
                        if (Math.abs(diffScenarios[i]) == 1 && i != 0) {
                            pendingOutput = pendingOutput + " goal, ";
                        } else {
                            if (i != 0) {
                                pendingOutput = pendingOutput + " goals, ";
                            } else {
                                pendingOutput = pendingOutput + ", ";
                            }
                        }
                    ...
                    pendingOutput = pendingOutput + " If the match ends with " + diffScenarios[i];
                    if (i == 0) {
                        if (Math.abs(diffScenarios[i]) != 1) {
                            pendingOutput = pendingOutput + " goals or less, ";
                        } else {
                    ...
                    if (i != 0) {
                        if (diffScenarios[i] == 1 && i != (diffScenarios.length - 1)) {
                            pendingOutput = pendingOutput + " goal, ";
                        } else {
                            if (i != (diffScenarios.length - 1)) {
                                pendingOutput = pendingOutput + " goals, ";
